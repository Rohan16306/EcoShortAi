...
// Rate limit presets using express-rate-limit
const authRateLimit = rateLimit({ windowMs: 15 * 60 * 1000, max: 10, message: { error: 'Too many requests. Please wait before trying again.' } });
const chatRateLimit = rateLimit({ windowMs: 60 * 1000, max: 20, message: { error: 'Too many requests. Please wait before trying again.' } });
const communityRateLimit = rateLimit({ windowMs: 60 * 1000, max: 5, message: { error: 'Too many requests. Please wait before trying again.' } });
const contactRateLimit = rateLimit({ windowMs: 60 * 1000, max: 3, message: { error: 'Too many requests. Please wait before trying again.' } });
...
// ============================================================
// PERF FIX: In-Memory Token Cache (60s TTL) + 500ms PocketBase Timeout
// Eliminates network roundtrip to PocketBase on every authenticated request.
// ============================================================
const tokenCache = new Map(); // token -> { userId, expiresAt }
const TOKEN_CACHE_TTL_MS = 60 * 1000; // 1 minute
const PB_AUTH_TIMEOUT_MS = 500; // Max wait for PocketBase before giving up

// Periodically prune expired cache entries (every 2 minutes)
setInterval(() => {
  const now = Date.now();
...
/**
 * Verifies a Bearer token against PocketBase with caching.
 * Returns the userId string on success, or null on failure.
 * - First call: hits PocketBase (with 500ms timeout), caches result for 60s.
 * - Subsequent calls within TTL: returns from cache (<1ms, zero network).
 * - Falls back to local JWT verify if PocketBase is unavailable.
 */
async function verifyTokenCached(token) {
  ...
  try {
    ...
    if (pbRes.ok) {
      ...
      const userId = pbData.record.id;
      tokenCache.set(token, { userId, expiresAt: now + TOKEN_CACHE_TTL_MS });
      return userId;
    }
  } catch (pbErr) {
    clearTimeout(timeoutId);
...
async function authMiddleware(req, res, next) {
  ...
  const userId = await verifyTokenCached(token);
...
async function extractUserIdFromAuth(req) {
  ...
  return verifyTokenCached(token);
}
