// --- STATE MANAGEMENT ---
        ...
        let _cachedGeoTag = null;
        let _geoTagCacheTime = 0;
        const GEO_CACHE_TTL_MS = 30000; // Cache geolocation for 30 seconds between scans
        let stream = null;
        ...
        function resolveApiBase() {
            if (isLocalhost()) return 'http://localhost:3002/api';
        ...
        document.addEventListener('DOMContentLoaded', async () => {
            ...
            updateGlobalStats();
            Promise.all([
                refreshGlobalImpact().catch(console.error),
                loadFeaturedMedia().catch(console.error),
            ...
            // Parse initial URL hash and load immediately
            const initialView = window.location.hash.replace('#', '') || 'home';
            setTimeout(() => router(initialView, true), 0);
        });

        const CLASS_NAMES = ['cardboard', 'glass', 'metal', 'paper', 'plastic', 'trash'];

        // ─── Lazy TensorFlow.js Loader ────────────────────────────────────────────────
        // TF.js scripts are NOT loaded at page start (they're ~2-4MB).
        // They are injected on-demand the first time the scan feature is used.
        ...
        function ensureTensorflowLoaded() {
            ...
            _tfLoadPromise = new Promise((resolve, reject) => {
                ...
                tfScript.onload = () => {
                    console.log('[AI] TensorFlow.js loaded on-demand.');
                    resolve();
                };
        ...
        function startModelLoad() {
            ...
            // Ensure TF.js is loaded before proceeding (lazy-loads TF.js on first scan use)
            return ensureTensorflowLoaded().then(() => {
                if (model) return model;
                if (modelLoadPromise) return modelLoadPromise;

                // Load the custom-trained local model
                modelLoadPromise = tf.loadGraphModel('/model/model.json')
                    .then((loadedModel) => {
                        model = {
                            classify: async (imgElement) => {
                                // Use tf.tidy for intermediate tensors, async readback avoids main thread block
        ...
        function updateGeoStatus(message, type = 'info') {
            ...
            const icons = {
                info: 'fa-location-crosshairs animate-spin',
                success: 'fa-map-pin',
                warning: 'fa-map-pin',
                error: 'fa-location-dot'
            };

            container.className = `mt-4 flex items-center justify-center gap-2 py-2 px-4 rounded-full text-sm mx-auto w-fit transition-colors duration-300 ${bgClasses[type]} ${textClasses[type]}`;
        ...
        function getCurrentGeoTag() {
            ...
            // Return cached geotag if still fresh (avoids repeated 3s waits)
            const now = Date.now();
            if (_cachedGeoTag && (now - _geoTagCacheTime) < GEO_CACHE_TTL_MS) {
                return Promise.resolve({ ..._cachedGeoTag, timestamp: new Date().toISOString() });
            }
            ...
            return new Promise((resolve) => {
                ...
                navigator.geolocation.getCurrentPosition(
                    (position) => {
                        ...
                        // Cache for subsequent rapid scans
                        _cachedGeoTag = tag;
                        _geoTagCacheTime = Date.now();
                        resolve(tag);
                    },
                    () => {
                        if (isResolved) return;
        ...
        function router(viewName, skipPushState = false) {
            try {
                ...
                const authRequired = ['dashboard', 'rewards', 'pickup', 'admin', 'collector'];
                ...
                // Native Admin View is now used directly

                document.querySelectorAll('.view-section').forEach(el => el.classList.remove('active'));
                const targetView = document.getElementById('view-' + viewName);
                if (targetView) targetView.classList.add('active');
                
                const floatingBtn = document.getElementById('sustain-floating-btn');
                if (floatingBtn) {
                    floatingBtn.style.display = viewName === 'ai' ? 'none' : 'flex';
                }
...
// --- REWARDS SYSTEM ---
        ...
        async function analyzeImage() {
            ...
            // PERF FIX: Run validate-image, geotag, classify, and DOM paint ALL in parallel.
            // Previously these ran sequentially: validate(~6s) → geo(~3s) → delay(350ms) → classify(~0.1s) = ~10s
            // Now they run in parallel: max(validate, geo, classify) ≈ ~6s (or ~0.1s on cache hit)
            let valRes = null;
            let predictions;
            let duplicate;
            ...
            try {
                const [validateResult, geoTag, classifyResult] = await Promise.all([
                    // 1. Validate image authenticity (backend Gemini call)
                    apiRequest('/scan/validate-image', {
                        method: 'POST',
                        ...
                        return { isAuthentic: true, reason: "Validation bypassed" };
                    }),
                    // 2. Get geolocation (uses cache for rapid consecutive scans)
                    getCurrentGeoTag(),
                    // 3. Run AI classification immediately (no need to wait for validation)
                    model.classify(imgElement)
                ]);

                valRes = validateResult;
            ...
            // Check validation result AFTER parallel execution
        ...
        async function analyzeVideo() {
            ...
            // PERF FIX: Run all independent operations in parallel
            let valRes = null;
            let videoHash;
            let duplicate;
            let locationDuplicate;
            ...
            try {
                const [validateResult, hashResult, geoTag, classifyResult] = await Promise.all([
                    apiRequest('/scan/validate-image', {
                        method: 'POST',
                    ...
                    generateImageHash(frameDataUrl),
                    getCurrentGeoTag(),
                    model.classify(imgElement)
                ]);

                valRes = validateResult;
                videoHash = hashResult;
                currentGeoTag = geoTag;
                predictions = classifyResult;
            } catch (err) {
                console.error('Step 2 parallel scan error:', err);
                alert('Failed to analyze Step 2 image: ' + err.message);
                document.getElementById('loading-area').classList.add('hidden');
                return;
            }
            ...
            try {
                duplicate = isDuplicateImage(videoHash);
                locationDuplicate = findDuplicateLocation(currentGeoTag);
                videoEvaluation = getEvaluationFromPredictions(predictions, duplicate, locationDuplicate);
                ...
                if (verifyResponse.matched) {
                    ...
                    const awarded = Number(verifyResponse.awardedCredits) || 0;
                    document.getElementById('earned-credits').innerText = awarded;
                    ...
                    if (awarded > 0 && typeof window.showCreditAnimation === 'function') {
                        window.showCreditAnimation(awarded);
                    }
                } else {
                    await commitVerifiedResult(
                        pendingVerification.photoEvaluation,
        ...
        function renderAiSidebarHistory() {
            ...
            if (userQueries.length === 0) {
                historyList.innerHTML = '<p class="text-sm text-gray-700 italic p-4">No recent history.</p>';
                return;
            }
                    ...
                    listEl.innerHTML = data.items.map(p => `
                        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex flex-col md:flex-row justify-between items-center hover:shadow-md transition">
                            <div class="mb-4 md:mb-0">
                                <h4 class="font-bold text-gray-800"><i class="fa-solid fa-recycle text-green-500 mr-2"></i>${escapeHtml(p.material)} (${escapeHtml(p.weight)} kg)</h4>
                                <p class="text-sm text-gray-600 mt-1">${escapeHtml(new Date(p.created).toLocaleDateString())} - ${escapeHtml(p.location_address)}</p>
                            </div>
                            <div>
        ...
        // These functions are defined inside DOMContentLoaded (local scope) but are
        // called via HTML onclick= attributes which require global (window) scope.
        // Without these, all navigation, login, and button clicks silently fail.
        ...
        window.throwLeaves          = typeof throwLeaves === 'function' ? throwLeaves : function(){}; // defined in ui-animations.js
        ...
        window.resetToNewScan       = resetToNewScan;
        window.clearHistory         = clearHistory;
        window.dismissDuplicateWarning = dismissDuplicateWarning;
        window.closeRewardModal     = closeRewardModal;
        window.copyRedeemCode       = copyRedeemCode;
        window.renderAdminDashboard = typeof renderAdminDashboard === 'function' ? renderAdminDashboard : function(){};
        // ─────────────────────────────────────────────────────────────────────────────
