// --- STATE MANAGEMENT ---
        ...
        // Reward Definitions
        const BADGES = [
            { id: 'starter', name: 'Eco Starter', icon: 'ðŸŒ±', description: 'Scan your first item', requirement: 1, type: 'scans', color: 'badge-bronze' },
            { id: 'recycler', name: 'Recycler', icon: 'â™»ï¸', description: 'Scan 10 items', requirement: 10, type: 'scans', color: 'badge-bronze' },
            { id: 'warrior', name: 'Green Warrior', icon: 'ðŸ›¡ï¸', description: 'Scan 50 items', requirement: 50, type: 'scans', color: 'badge-silver' },
            { id: 'champion', name: 'Eco Champion', icon: 'ðŸ‘‘', description: 'Scan 100 items', requirement: 100, type: 'scans', color: 'badge-gold' },
            { id: 'master', name: 'Master Recycler', icon: 'ðŸ’Ž', description: 'Scan 500 items', requirement: 500, type: 'scans', color: 'badge-platinum' },
            { id: 'legend', name: 'Eco Legend', icon: 'ðŸŒ', description: 'Scan 1000 items', requirement: 1000, type: 'scans', color: 'badge-diamond' }
        ];

        const REWARDS = [
            { id: 'coffee', name: 'Free Coffee', description: 'Get a free coffee at participating cafes', cost: 50, icon: 'â˜•', type: 'redeem' },
            { id: 'tote', name: 'Eco Tote Bag', description: 'Reusable shopping bag with EcoSort logo', cost: 100, icon: 'ðŸ›ï¸', type: 'redeem' },
            { id: 'bottle', name: 'Steel Water Bottle', description: 'Premium insulated water bottle', cost: 200, icon: 'ðŸ¥¤', type: 'redeem' },
            { id: 'amazon10', name: '$10 Amazon Gift Card', description: 'Digital gift card for Amazon', cost: 300, icon: 'ðŸŽ', type: 'code' },
            { id: 'starbucks20', name: '$20 Starbucks Card', description: 'Digital Starbucks gift card', cost: 500, icon: 'â˜•', type: 'code' },
            { id: 'plant', name: 'Free Plant Kit', description: 'Grow your own herbs at home', cost: 150, icon: 'ðŸŒ¿', type: 'redeem' },
            { id: 'shirt', name: 'EcoSort T-Shirt', description: 'Organic cotton branded t-shirt', cost: 400, icon: 'ðŸ‘•', type: 'redeem' },
            { id: 'amazon50', name: '$50 Amazon Gift Card', description: 'Digital gift card for Amazon', cost: 1000, icon: 'ðŸŽ‰', type: 'code' }
        ];
        ...
        document.addEventListener('DOMContentLoaded', async () => {
            ...
            // Start model load in background to reduce first-scan wait time.
            startModelLoad();
        });
        ...
        function calculateQuizResults() {
            ...
            const messages = {
                4: 'Perfect! You are a true Eco Champion! ðŸŒŸ',
                3: 'Great job! You know your recycling facts! ðŸŒ±',
                2: 'Good effort! Keep learning about sustainability! ðŸ“š',
                1: 'Keep trying! Every bit of knowledge helps the planet! ðŸŒ',
                0: 'Don\'t worry! Now you know the facts - time to take action! ðŸ’ª'
            };
        ...
        function router(viewName) {
            if ((viewName === 'dashboard' || viewName === 'detect' || viewName === 'rewards') && !currentUser) {
                openAuthModal('login');
...
// --- REWARDS SYSTEM ---
        ...
        function renderBadges() {
            ...
            BADGES.forEach(badge => {
                ...
                badgeEl.className = `reward-card bg-white p-6 rounded-xl shadow-sm text-center ${!isUnlocked ? 'locked' : ''}`;
                
                badgeEl.innerHTML = `
                    <div class="badge-icon ${isUnlocked ? badge.color : 'bg-gray-200'} mb-3">
                    ...
                    <div class="text-sm font-medium ${isUnlocked ? 'text-green-600' : 'text-gray-400'}">
                        ${isUnlocked ? (isClaimed ? 'âœ“ Claimed' : 'Unlocked!') : `${scanCount}/${badge.requirement} scans`}
                    </div>
                `;
                
                if (isUnlocked && !isClaimed) {
                    badgeEl.innerHTML += `
                        <button onclick="claimBadge('${badge.id}')" class="mt-3 w-full bg-green-600 text-white py-2 rounded-lg text-sm font-medium hover:bg-green-700 transition">
                            Claim Badge
                        </button>
                    `;
                }
        ...
        function renderRewards() {
            ...
            REWARDS.forEach(reward => {
                ...
                rewardEl.className = `reward-card bg-white p-6 rounded-xl shadow-sm ${!canAfford || isClaimed ? 'locked' : ''}`;
                ...
                rewardEl.innerHTML = `
                    ...
                    <h4 class="font-bold text-gray-900 mb-2">${reward.name}</h4>
                    <p class="text-sm text-gray-600 mb-4">${reward.description}</p>
                    <div class="flex items-center justify-between">
                        <span class="text-xs text-gray-500">
                            ${reward.type === 'code' ? 'ðŸŽŸï¸ Redeem Code' : 'ðŸ“¦ Physical Item'}
                        </span>
                        ${isClaimed ? 
                            '<span class="text-green-600 font-medium text-sm">âœ“ Claimed</span>' :
                            `<button onclick="claimReward('${reward.id}')" class="px-4 py-2 ${canAfford ? 'bg-green-600 hover:bg-green-700 text-white' : 'bg-gray-200 text-gray-400 cursor-not-allowed'} rounded-lg text-sm font-medium transition" ${!canAfford ? 'disabled' : ''}>
        ...
        function renderClaimedRewards() {
            ...
            sorted.forEach(reward => {
                ...
                row.innerHTML = `
                    <td class="px-6 py-4">
                        <div class="flex items-center">
                            <span class="text-2xl mr-3">${rewardInfo.icon}</span>
                            <span class="font-medium text-gray-900">${rewardInfo.name}</span>
                        </div>
                    </td>
                    <td class="px-6 py-4">
                        <span class="px-2 py-1 rounded text-xs font-medium ${isCode ? 'bg-purple-100 text-purple-800' : 'bg-blue-100 text-blue-800'}">
                            ${isCode ? 'Redeem Code' : 'Badge/Item'}
                        </span>
                    </td>
                    <td class="px-6 py-4 text-gray-600">${date}</td>
                    <td class="px-6 py-4">
                        <span class="px-2 py-1 rounded text-xs font-medium bg-green-100 text-green-800">Claimed</span>
                    </td>
        ...
        function showRewardModal(item, type, code = null) {
            ...
            document.getElementById('reward-icon').textContent = item.icon;
            ...
            if (type === 'reward' && code) {
                ...
                document.getElementById('badge-display').innerHTML = item.icon;
        ...
        function showRedeemCode(code, name) {
            const modal = document.getElementById('reward-modal');
            document.getElementById('reward-icon').textContent = 'ðŸŽŸï¸';
        ...
        function getVideoFrameDataUrl(video) {
            const canvas = document.createElement('canvas');
        ...
        async function loadImageElement(src) {
            return new Promise((resolve, reject) => {
                const img = new Image();
                img.onload = () => resolve(img);
                img.onerror = () => reject(new Error('Failed to load captured video frame'));
                img.src = src;
            });
        }
        ...
        function getEvaluationFromPredictions(predictions, duplicate = null, locationDuplicate = null) {
            ...
            const finalCredits = isDuplicate || isLocationDuplicate ? 0 : credits;
        ...
        function renderScanResult(predictions, evaluation, duplicate = null, locationDuplicate = null, geoTag = null) {
            ...
            if (evaluation.isDuplicate || evaluation.isLocationDuplicate) {
                ...
                if (evaluation.isDuplicate) {
                    ...
                    const detail = duplicateBanner.querySelector('p.text-sm');
                    if (detail) {
                        const oldDate = new Date(locationDuplicate.date).toLocaleString();
                        detail.textContent = `Same scan location detected (${Math.round(locationDuplicate.distance)}m away from scan on ${oldDate}).`;
                    }
                    document.getElementById('result-duplicate-date').textContent = '';
                    updateGeoStatus('Same-location geotag detected. Credits blocked.', 'warning');
                }
            } else {
            ...
            document.getElementById('credit-reason').innerText = 'Pending Step 2 video verification.';
        ...
        async function startBackendVerificationSession(photoEvaluation) {
            try {
                const payload = await apiRequest('/scan/verify/start', {
                    ...
                    body: JSON.stringify({
                        photo: {
                            ...
                            duplicateBlocked: photoEvaluation.isDuplicate || photoEvaluation.isLocationDuplicate
                        }
                    })
                });
        ...
        async function completeBackendVerificationSession(verificationId, videoEvaluation) {
            return apiRequest('/scan/verify/complete', {
                method: 'POST',
                body: JSON.stringify({
                    verificationId,
                    video: {
                        label: videoEvaluation.itemName,
                        material: videoEvaluation.material,
                        isRecyclable: videoEvaluation.isRecyclable
                    }
                })
            });
        }
        ...
        async function commitVerifiedResult(photoEvaluation, geoTag, awardedCredits) {
            ...
            const historyItem = {
                ...
                isDuplicate: photoEvaluation.isDuplicate || photoEvaluation.isLocationDuplicate,
                duplicateType: photoEvaluation.isDuplicate ? 'image' : (photoEvaluation.isLocationDuplicate ? 'location' : null),
                geoTag: geoTag
            };
            ...
            } else if (photoEvaluation.isLocationDuplicate) {
                showNotification('Same location detected - no credits awarded', 'warning');
        ...
        async function analyzeVideo() {
            ...
            try {
                ...
                const sampleTime = Number.isFinite(video.duration) && video.duration > 0
                    ? Math.min(video.duration / 2, Math.max(video.duration - 0.1, 0))
                    : 0;
                await seekVideo(video, sampleTime);

                const frameDataUrl = getVideoFrameDataUrl(video);
                currentImage = frameDataUrl;
                currentImageHash = await generateImageHash(currentImage);
                ...
                const imgElement = await loadImageElement(frameDataUrl);
                const predictions = await model.classify(imgElement);
                ...
                updateVideoGeoStatus('Video frame analyzed successfully.', 'success');
                ...
                if (verifyResponse.matched) {
                    ...
                    document.getElementById('credit-reason').innerText = 'Photo and video did not match.';
                    setVerificationBanner('warning', `<strong>Verification failed.</strong> ${verifyResponse.reason}`);
                }
                pendingVerification = null;
                ...
                document.getElementById('loading-area').classList.add('hidden');
                document.getElementById('video-preview-area').classList.remove('hidden');
                showNotification('Could not analyze this video. Try another one.', 'error');
            }
        }
